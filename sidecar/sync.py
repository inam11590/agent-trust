...
def sync_configuration_sync() -> bool:
    ...
    endpoints = [config.control_plane_url]
    if config.secondary_control_plane_url:
        endpoints.append(config.secondary_control_plane_url)

    for base_url in endpoints:
        url = f"{base_url.rstrip('/')}/v1/gateways/{state.gateway_id}/config?client_version={state.applied_config_version}"
        ...
        try:
            ...
            if http_err.code == 304:
                ...
                state.last_sync_time = datetime.now(timezone.utc)
                return False
            logger.warning(f"Endpoint {base_url} returned HTTP {http_err.code}")
        except Exception as exc:
            logger.warning(f"Endpoint {base_url} unreachable for config sync: {exc}")
...
def send_heartbeat_sync() -> bool:
    ...
    for base_url in endpoints:
        url = f"{base_url.rstrip('/')}/v1/gateways/{state.gateway_id}/heartbeat"
        ...
        try:
            ...
            logger.warning(f"Heartbeat to {base_url} failed: {exc}")
